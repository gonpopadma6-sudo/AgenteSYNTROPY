...
def update_scaffolding_level(user_history):
    ...
    time_in_level = count_iterations_in_current_level()

    # Self-Fading Scaffolding: Redução metódica evita RC-05 (A Armadilha da Especificação PBL)
    if avg_score > 8.0 and current_level < 4:
        trigger_fade_out(target_mode="Socratic_or_Silence") # Expande para Nível mais autônomo.
    elif avg_score >= 6.0 and time_in_level > MAX_TIME_IN_LEVEL:
        # Fading compulsório inercial: Empurra o Host ao desconforto criativo OOD (Sintropia Limpa).
        trigger_forced_fade_out()
    elif avg_score < 4.0 and current_level > 1:
        trigger_scaffold_up() # Regression (Suporte Tático Emergencial Volátil).
```
