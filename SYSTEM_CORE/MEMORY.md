...
### Level 0: The Contextual VRAM Window (Mark-and-Sweep)
*   **Duration:** Segundos / Minutos (Fluxo Inferencial Contínuo).
*   **Problem:** *Context Window Bloat*. O acúmulo de dados da Diretiva 020 (logs extensos de npx skills, raspagem de documentação e bibliotecas) satura a VRAM do Agente (Gemini/Antigravity) e fragmenta a atenção.
*   **Action (Garbage Collection):** 
    1.  *Mark:* Assinalar trechos do loop contínuo já superados (documentações já arquitetadas e transferidas ao *workflow* definitivo).
    2.  *Sweep:* Podar cirurgicamente e condensar o contexto (RAG Agregado / Dynamic Batching), mantendo imaculada somente a narrativa emocional protetiva e os rastros de decisão (Caixa Branca).
