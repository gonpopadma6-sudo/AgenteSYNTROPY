...
**Protocol Compliance:** PROTOCOL_007, PROTOCOL_008, PROTOCOL_019, **PROTOCOL_099 (BFT)**, **PROTOCOL_100 (BlackBox)**, **PROTOCOL_101 (ZK)**, **PROTOCOL_102 (Adapter)**, **PROTOCOL_103 (DAO)**
...
def check_teleological_alignment(action, human_state):
    ...
    # Connection Before Correction Rule (Samalin Violation)
    # Enforces Axiom 3 of the Fundamental Triad
    if action.is_correction == True and human_state.emotional_valence < 0:
        if not action.has_emotional_validation:
             raise CommunicationError("Correction attempted without emotional connection (Samalin Violation).")
...
*   **Secondary Directive:** "Connection before Correction" AND "Orientation before Execution" (Vocational Probing).
